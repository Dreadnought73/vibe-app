...
export const codeAgentFunction = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const codeAgent = createAgent<AgentState>({
      ...
      tools: [
        ...
        createTool({
            ...
            handler: async({files}, {step, network}: Tool.Options<AgentState>) => {
              const newFiles = await step?.run("createOrUpdateFiles", async() => {
                try {
                  // this is to keep track of the modified files
              ...
              // we only store objects in our internal network state
        ...
        createTool({
          ...
          // should always extract the step from the tool
          handler: async({files}, {step}) => {
            return await step?.run("readFiles", async() => {
              try {
                ...
                // this is for the AI, so the output format does not matter much
      ...
      // it will go in a cycle using the tools, it will create its own plan
      lifecycle: {
    ...
    await step.run("save.result", async() => {
      ...
      return await prisma.message.create({
        data: {
          // anytime a message is created project ID is added
          projectId: event.data.projectId,
